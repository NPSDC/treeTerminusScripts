...
source("getCandU.R")
# load("environment/no_threshold/swish_y.RData")
load("environment/no_threshold/tree.RData")
library(treeclimbR)
library(ggplot2)
library(tictoc)
...
# save(yAll, file = "environment/no_threshold/ySwishAllNodes.RData")
# load("environment/no_threshold/ySwishAllNodes.RData")
# # 
# swishRes <- data.frame(mcols(yAll))
# swishRes$node <- seq(1:nrow(yAll))
# swishRes <- swishRes[,-c(2)] ### Removing keep
# swishRes <- share(swishRes)
# gc()
...
# toc()
# gc()
# save(cSwish, file = "environment/no_threshold/cSwish.RData")
# 
...
# save(bSwish, file = "environment/no_threshold/bSwish.RData")
# gc()

#### Stratifying by inferential variance
# infRVs <- mcols(yAll)[,"meanInfRV"]
# infCuts <- cut_number(infRVs, 3)
...
# save(swishL, file = "environment/no_threshold/swish_infRV.RData")
# gc()
# 
load("environment/no_threshold/swish_infRV.RData")
tic()
cSwishInf <- getCandU(tree,
                     ...
                     cores = 3)
toc()
save(cSwishInf, file = "environment/no_threshold/c_swish_infRV.RData")
...
save(bSwishInf, file = "environment/no_threshold/bSwishInfRV.RData")
gc()