...
yAll <- labelKeep(yAll)
set.seed(100)
print(system.time(tBeforeQval <- runTreeTermAlphas(tree, yAll, "condition", 0.20, pCutOff = 0.05, pChild = 0.05, runType = c("b"), corr = c("qvalue"), alphas = c(0.01, 0.05, 0.10), cores = 3, compPThresh = T, cSign = T, temp=T)))
save(tBeforeQval, file = "environment/brain_sim/final1_NA/tBeforeQval.RData")
```