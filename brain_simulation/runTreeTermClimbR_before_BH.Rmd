...
```{r}
print("tbeforeBH")
print(system.time(tbeforeBH <- runTreeTermAlphas(tree, yAll, "condition", minInfRV, pCutOff = 0.05, pChild = 0.05, runType = c("b"), corr = c("BH"), alphas = c(0.01, 0.05, 0.10), cores = 3, compPThresh = T, temp=T)))
save(tbeforeBH, file = "environment/brain_sim/tbeforeBH.RData")

print("tbeforeBHNoStop")
print(system.time(tbeforeBHNoStop <- runTreeTermAlphas(tree, yAll, "condition", -100000, pCutOff = 0.05, pChild = 0.05, runType = c("b"), corr = c("BH"), alphas = c(0.01, 0.05, 0.10), cores = 3, compPThresh = T, cSign = F, temp=T)))
save(tbeforeBHNoStop, file = "environment/brain_sim/tbeforeBHNoStop.RData")

print("tbeforeBHNoThresh")
print(system.time(tbeforeBHNoSign <- runTreeTermAlphas(tree, yAll, "condition", 0.20, pCutOff = 0.05, pChild = 0.05, runType = c("b"), corr = c("BH"), alphas = c(0.01, 0.05, 0.10), cores = 3, compPThresh = T, cSign = F, temp=T)))
