...
trCorUPGMA <- list()
for(i in seq(12)) {
    corTxps <- cor(t(infReps[,i,]))/2 ###Building on anticorrelation
    dd <- (1-corTxps)/2
    rm(corTxps)
    gc()
    dd <- 1-dd
...
save(trCorUPGMA, file="environment/brain_sim_nodtu/mode=gc_bias/post_type=gibbs_nrep=100_tf=100/comp_trees/trCorUPGMA.RData")
