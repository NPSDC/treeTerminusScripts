...
nNodes <- length(trMeanCorUPGMA$tip)+trMeanCorUPGMA$Nnode
desc <- Descendants(trMeanCorUPGMA, seq(nNodes))
genesCor <- mclapply(desc[length(trMeanCorUPGMA$tip)+1:trMeanCorUPGMA$Nnode], function(txps) unique(mapDf[rownames(yCorMeanAggS)[txps],1]), mc.cores=4)
