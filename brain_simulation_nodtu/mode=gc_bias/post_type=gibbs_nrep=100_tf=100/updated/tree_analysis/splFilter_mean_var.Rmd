...
for(i in seq_along(treeListNoThr)){
    ...
    print("Agg completed")
    spl <- compSPL(yAgg, i=i, mv=F, meanV=T)
    print("spl completed")
    ...
    treeNoThrCuts[[i]] <- findCuts(tr, globArr, spl, nrow(yR)+1,lengths)
    if(length(treeNoThrCuts[[i]]) == 1) {
      print(paste("NoThr",i))
      treeNoThrCuts[[i]] <- unlist(lapply(children, function(node) findCuts(tr, globArr, spl, node, lengths)))
    }

    print("opt cuts found")
    dfSPL <- rbind(dfSPL, data.frame(spl = spl, optSPL = globArr, sample = samples[i]))
    dComp <- data.frame("Sample"=samples[i],"TreeTerm_opt"=sum(globArr[treeNoThrCuts[[i]]]),
                "TreeTerm_groups" = sum(spl[children]*lengths[children]),
                "Transcripts" = sum(spl[1:nrow(yR)]))
    dInd <- rbind(dInd, data.frame(SPL=globArr[treeNoThrCuts[[i]]], Sample=samples[i], Type="TreeTerm_opt"), data.frame(SPL=spl[children]*lengths[children], Sample=samples[i], Type="TreeTerm_groups"), data.frame(SPL=sum(spl[1:nrow(yR)]), Sample=samples[i], Type="Transcripts"))
    ### Hirak terminus
    ...
    treeThrCuts[[i]] <- findCuts(trThr, globArr, spl, nrow(yR)+1,lengths)
    print("opt SPL completed")
    if(length(treeThrCuts[[i]]) == 1) {
      print(paste("Thr",i))
      treeThrCuts[[i]] <- unlist(sapply(childrenThr, function(node) findCuts(trThr, globArr, spl, node, lengths)))
    }

    print("opt cuts found")

    dComp <- cbind(dComp, data.frame("Term_opt"=sum(globArr[treeThrCuts[[i]]]),
                "Term_groups" = sum(spl[childrenThr]*lengths[childrenThr])))
    dfSPLComp <- rbind(dfSPLComp, dComp)
    dInd <- rbind(dInd, data.frame(SPL=globArr[treeThrCuts[[i]]], Sample=samples[i], Type="Term_opt"),
                  data.frame(SPL=spl[childrenThr]*lengths[childrenThr], Sample=samples[i], Type="Term_groups"))
}
save(dInd, file = "environment/brain_sim_nodtu/mode=gc_bias/post_type=gibbs_nrep=100_tf=100/updated/comp_trees/dInd_mv.RData")
```

```{r}
save(dfSPLComp, file = "environment/brain_sim_nodtu/mode=gc_bias/post_type=gibbs_nrep=100_tf=100/updated/comp_trees/dfSPLComp_mv.RData")
save(dfSPL, file = "environment/brain_sim_nodtu/mode=gc_bias/post_type=gibbs_nrep=100_tf=100/updated/comp_trees/dfSPL_mv.RData")
save(treeNoThrCuts, file = "environment/brain_sim_nodtu/mode=gc_bias/post_type=gibbs_nrep=100_tf=100/updated/comp_trees/treeNoThrCuts_mv.RData")
save(treeThrCuts, file = "environment/brain_sim_nodtu/mode=gc_bias/post_type=gibbs_nrep=100_tf=100/updated/comp_trees/treeThrCuts_mv.RData")
save(trNThrList, file = "environment/brain_sim_nodtu/mode=gc_bias/post_type=gibbs_nrep=100_tf=100/updated/comp_trees/trNThrList_mv.RData")
save(trThrList, file = "environment/brain_sim_nodtu/mode=gc_bias/post_type=gibbs_nrep=100_tf=100/updated/comp_trees/trThrList_mv.RData")
```
