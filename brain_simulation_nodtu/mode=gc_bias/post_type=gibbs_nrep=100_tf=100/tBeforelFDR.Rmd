...
```{r}
print(system.time(tBeforelFDR <- runTreeTermAlphas(tree, yAll, "condition", 0.20, pCutOff = 0.05, pChild = 0.05, runType = c("b"), corr = c("lfdr"), alphas = c(0.01, 0.05, 0.10), cores = 3, compPThresh = T, temp=T, cSign=F, minP = 0.80)))
save(tBeforelFDR, file = "environment/brain_sim_nodtu/mode=gc_bias/post_type=gibbs_nrep=100_tf=100/tBeforelFDR.RData")
```